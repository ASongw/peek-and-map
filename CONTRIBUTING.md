...
Map View 提供基于 HTML5 Canvas 2D 的图形视图，通过「View」按钮在树形列表和图形之间切换。图形使用 BFS 层级布局算法，从左到右排列，Bezier 曲线连边并带箭头。节点统一为圆角矩形，通过符号名前的**彩色字母徽章**区分类型（如 `f` Function、`m` Method、`C` Class、`I` Interface、`v` Variable、`S` Struct 等），徽章颜色与节点边框颜色一致，颜色来源于当前主题的 `--peek-kind-*` CSS 变量。函数参数自动去除，保持标签简洁。支持滚轮缩放（0.2x–5x）、鼠标拖拽平移、ResizeObserver 自动重布局。

**15a. 图形视图节点合并**

当同一封闭符号（如函数 B）在多处引用了目标符号（如函数 A）时，树形列表中会显示多个节点（首个可展开，后续为叶子节点），但图形视图通过 `mergeItemsBySymbol()` 函数自动将这些重复节点合并为一个。合并后的节点在不同位置的引用行号各自显示为可交互的行号徽章（如 `L10` `L20` `L30`），排列在符号名称下方。节点高度根据是否有多个调用位置自动调整（单调用位置：32px，多调用位置：32+16=48px）。鼠标悬停在行号徽章上时高亮显示，单击触发 `peekOnly`（预览），双击触发 `jumpTo`（跳转到编辑器），行为与树形视图中对应的节点完全一致。合并使用符号定义位置（`uri + line + character`）作为去重键，保留首个可展开的 `nodeId` 用于递归展开。
...
- **Ctrl/Cmd+点击**（图形视图）：展开或折叠子节点，触发 `expandRef`/`expandIncoming` 请求或本地折叠
