...
────────────────              ────────────────────────────────      ─────────────────────
                 ──消息──▶  onDidReceiveMessage({ type:'search', includeGlob, excludeGlob })
                              │  _doSearch()
                              │    ├─ 获取光标处单词
                              │    ├─ executeReferenceProvider → 引用列表
                              │    │  _resolveReferencingSymbols()
                              │    │  （为每条引用查找其封闭符号，按 include/exclude 过滤并构建引用树）
...
点击 "Files ▸"                            ──▶  展开/收起 include/exclude 输入框（默认收起）
```
...
Map View 的 Analysis 使用 Emoji 图标（🔍）；视图切换使用文字标签（`Outline` / `Graph`），Graph 模式下提供方向下拉框（`Right` / `Left` / `Up` / `Down`）。Outline 视图的表头与行内容采用统一列布局，列分割线可拖动；分割线位于列之间并贯穿整行，背景与实例标签栏保持一致。

Map View 额外提供 `Files ▸` 按钮（默认收起），展开后显示 `files to include` 与 `files to exclude` 两个输入框。输入框支持 glob（可用逗号分隔多个模式），按 Enter 或点击 Analysis 即触发带过滤条件的分析。
...
**20. Map View 文件过滤（Include / Exclude）**

Map View 分析支持文件过滤：

- Header 的 `Files ▸` 默认隐藏 include/exclude 输入框，行为对齐 VS Code Search 的“附加过滤框”体验。
- 输入 include/exclude 后，`search` 消息会携带 `includeGlob` 与 `excludeGlob`。
- 扩展侧在 `_resolveReferencingSymbols()` 中对每条引用执行文件路径匹配（glob）；匹配失败的引用直接跳过。
- 过滤不仅影响首层结果，也会影响后续 `expandRef` 的递归展开。
- 过滤条件按实例保存：每个实例可拥有独立的 include/exclude 文本与过滤面板展开状态，切换实例自动恢复。
