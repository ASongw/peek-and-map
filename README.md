...
一个**AI纯度99%**的 VS Code 插件，提供 **Peek View** 和 **Map View** 两个视图（可自由拖动）：
...
| **编辑器代码跳转** | 在视图中双击空白区域或单击行号，编辑器即跳转到对应位置；双击代码文本则正常选中文字。 |
| **视图内跳转** | 在视图中按住 Ctrl 并点击函数/类型/宏等符号，直接在 Peek 视图内跳转到该符号的定义，无需离开视图。 |
| **前进/后退导航** | Ctrl+点击跳转后可通过**视图内**左上角按钮或鼠标侧键在历史记录中导航。 |
...
| 功能 | 说明 |
| ---- | ---- |
| **引用分析** | 点击「Analysis」按钮后，展示所有引用当前符号的封闭函数/类，按符号去重；递归展开时按“当前路径”去重，避免同一路径中重复出现已访问节点（防回路）。 |
...
| **图形视图（Graph）** | 图形视图以 BFS 树布局展示符号关系，Bezier 曲线连边；普通节点为圆角矩形，函数声明节点使用直角梯形边框（`Left` 方向自动镜像）；通过符号名前的彩色 Emoji 图标区分对象类型。|
...
| **图形视图交互** | 单击节点在 Peek View 中预览（preserveFocus）、双击在编辑器中打开、点击节点侧边 `+/-` 按钮展开/折叠子节点、拖拽平移；若某节点尝试展开后无子节点，其侧边按钮会变为空心圆提示“无可展开内容”；滚轮默认上下平移，`Shift+滚轮` 左右平移，鼠标滚轮左右拨动可左右平移，`Ctrl+滚轮` 缩放；等待动画显示在节点延伸方向（上/下/左/右）。 |
