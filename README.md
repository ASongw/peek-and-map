...
- **Peek View**：实时展示光标处符号的定义体，默认定位到定义起始位置（优先停在声明/定义开头，例如 `typedef` / `enum`），跟随编辑器语法高亮、支持视图内跳转、前进后退导航、锁定跟随和快速字体调整。
- **Map View**：分析光标处符号的引用关系，支持 `Outline` / `Graph` 两种视图；支持多实例标签并行分析（每个实例可独立切换视图与方向、文件过滤条件）；`Graph` 支持上下左右四个生长方向，支持递归展开和跳转。
- **Symbol Search**：在工作区内按关键字搜索符号（支持空格分隔多关键字 AND 匹配），输入时实时刷新结果列表；支持按文件范围限定 / 排除结果。
...
| 功能 | 说明 |
| ---- | ---- |
| **显示光标处符号的定义** | 光标选中符号时，自动展示该符号的**定义体**（函数/方法/类/结构体/宏等等）。当语言服务返回多个定义时，左侧会显示候选列表，可点击或用方向键切换。 |
| **实时跟随光标** | 光标在编辑器中移动时，视图内容自动刷新。 |
| **当前行高亮** | Peek 视图内高亮当前定位行，便于快速识别定义起始位置。 |
...
| **多定义候选列表** | 当 `Definition Provider` 返回多个结果时，Peek 视图左侧会列出所有候选；点击候选项会即时更新右侧预览，支持 `↑/↓/Home/End` 键切换。 |
...
| **Outline 列宽拖动** | `Outline` 视图的名称 / 文件 / 行号列之间提供可拖动分割线；|
...
| **文件包含/排除过滤** | Analysis 按钮旁新增 `Files ▸` 开关（默认收起），展开后可输入 `files to include` / `files to exclude`（glob，支持逗号分隔多个模式，如 `src/**,**/*.ts`）；过滤会同时作用于首层分析与后续递归展开。 |
...
| **实例过滤状态保持** | 每个分析实例会独立保存 `Files` 过滤面板展开状态与 include/exclude 输入内容；仅在当前会话内恢复，关闭或重载工作区后重置。 |
...
| **文件范围过滤** | 在排序下拉框左侧提供 `Files ▸` 开关，可输入 `files to include` / `files to exclude`（glob，支持逗号分隔多个模式，如 `src/**,**/*.ts`）来限定或排除结果；过滤状态仅在当前会话内保持。 |
